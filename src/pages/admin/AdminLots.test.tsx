...
vi.mock("convex/_generated/api", () => ({
  api: {
    ...
    auctions: {
      ...
      mutations: {
        publish: {
          closeLotEarly: "auctions/mutations/publish:closeLotEarly",
        },
        update: {
...
const mockAuctions = [
  {
    ...
    currentPrice: 150000,
    reservePrice: 140000,
    auctionEndTime: Date.now() + 86400000, // 1 day from now
  ...
  },
  {
    ...
    currentPrice: 0,
    reservePrice: 250000,
    auctionEndTime: Date.now() + 172800000, // 2 days from now
  ...
  },
  {
    ...
    currentPrice: 5000,
    reservePrice: 4000,
    auctionEndTime: Date.now() - 3600000, // 1 hour ago
...
describe("AdminLots", () => {
  ...
  beforeEach(() => {
    ...
    (useMutation as Mock).mockImplementation((name: string) => {
      if (name === "auctions/mutations/publish:closeLotEarly")
        return closeAuctionEarlyMock;
      if (name === "auctions/mutations/update:bulkUpdateLots")
  ...
  it("handles force ending an auction", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Close Lot Early?")).toBeInTheDocument();
    });

    const confirmButton = screen.getByRole("button", { name: "Close Lot" });
  ...
  it("handles bulk status update to approved", async () => {
    ...
    await waitFor(() => {
      expect(bulkUpdateAuctionsMock).toHaveBeenCalledWith({
        lotIds: ["auction1", "auction2", "auction3"],
        updates: { status: "approved" },
      });
      expect(toast.success).toHaveBeenCalledWith(
        expect.stringContaining("Updated 3 lots to approved")
      );
    });
  });
  ...
  it("renders empty state when no auctions match search", () => {
    ...
    expect(
      screen.getByText("No lots found matching your search.")
    ).toBeInTheDocument();
  });
      ...
      results: [
        {
          ...
          title: "Days Left",
          status: "assigned",
          auctionEndTime: now + 2 * 86400000 + 5 * 3600000, // 2 days 5 hours
          currentPrice: 100,
          reservePrice: 100,
          make: "m",
          model: "m",
        ...
        },
        {
          ...
          title: "Hours Left",
          status: "assigned",
          auctionEndTime: now + 5 * 3600000 + 10 * 60000, // 5 hours 10 mins
          currentPrice: 100,
          reservePrice: 100,
          make: "m",
          model: "m",
        ...
        },
        {
          ...
          title: "Minutes Left",
          status: "assigned",
          auctionEndTime: now + 10 * 60000, // 10 mins
          currentPrice: 100,
          reservePrice: 100,
          make: "m",
          model: "m",
        ...
        },
        {
          ...
          title: "Seconds Left",
          status: "assigned",
          auctionEndTime: now + 30 * 1000, // 30 secs
          currentPrice: 100,
          reservePrice: 100,
          make: "m",
          model: "m",
      ...
      results: [
        {
          ...
          title: "1 Day 1 Hour",
          status: "assigned",
          auctionEndTime: now + 86400000 + 3600000,
          currentPrice: 100,
          reservePrice: 100,
          make: "m",
          model: "m",
        ...
        },
        {
          ...
          title: "2 Days 1 Hour",
          status: "assigned",
          auctionEndTime: now + 2 * 86400000 + 3600000,
          currentPrice: 100,
          reservePrice: 100,
          make: "m",
          model: "m",
  ...
  it("handles close auction early with missing error message", async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith("Failed to close lot");
    });
  });
  ...
  it("displays dash when adminStats is missing", () => {
    ...
    renderComponent();
    expect(screen.getByText(/Showing 3 of — Lots/i)).toBeInTheDocument();
  });
  ...
  it("prevents closing dialog when isClosing is true", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText("Close Lot Early?")).not.toBeInTheDocument();
    });
  });
});
