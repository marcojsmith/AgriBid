    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
      <div className="bg-primary/5 border border-primary/10 rounded-lg p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold tracking-tight">Listing Summary</h3>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {PHOTO_SLOTS.map((slot) => {
            ...
            return (
              <div
                key={slot.id}
                className="aspect-video rounded-lg border overflow-hidden bg-muted relative group shadow-sm"
              >
                {storageId ? (
                ...
                <div className="absolute inset-x-0 bottom-0 bg-black/60 text-primary-foreground text-[8px] font-semibold p-1.5 text-center backdrop-blur-sm">
                  {slot.label}
                </div>
              </div>
            );
