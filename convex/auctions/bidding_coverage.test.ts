...
describe("Bidding Coverage", () => {
  ...
  describe("bidding.ts - placeBid", () => {
    it("should throw error if auction not found", async () => {
      ...
      await expect(placeBidHandler(mockCtx, { auctionId: "a1" as Id<"auctions">, amount: 100 }))
    ...
    it("should throw error if auction ended", async () => {
      vi.mocked(auth.requireVerified).mockResolvedValue({ userId: "u1" } as any);
      mockCtx.db.get.mockResolvedValue({ status: "active", sellerId: "u2", endTime: Date.now() - 1000 });
  ...
  describe("proxy_bidding.ts - getMyProxyBid", () => {
    it("should return null if not authenticated", async () => {
      ...
      const result = await getMyProxyBidHandler(mockCtx, { auctionId: "a1" as Id<"auctions"> });
  ...
  describe("proxy_bidding.ts - helper functions", () => {
    ...
    it("getCurrentHighestBidAmount should use most recent bid amount", async () => {
      ...
      const amount = await getCurrentHighestBidAmount(mockCtx, "a1" as Id<"auctions">);
      expect(amount).toBe(1500);
    });

    it("getCurrentHighestBidAmount should throw if auction not found", async () => {
      mockCtx.db.get.mockResolvedValue(null);
      await expect(getCurrentHighestBidAmount(mockCtx, "a1" as Id<"auctions">))
  ...
  describe("proxy_bidding.ts - handleNewBid and resolveProxyBids", () => {
    it("should handle first bid validation", async () => {
      const auction = { _id: "a1", currentPrice: 1000, minIncrement: 100, status: "active" };
      mockCtx.db.get.mockResolvedValue(auction);
      
      await expect(handleNewBid(mockCtx, "a1" as Id<"auctions">, "u1", 500))
    ...
    it("should handle subsequent bid validation", async () => {
      ...
      await expect(handleNewBid(mockCtx, "a1" as Id<"auctions">, "u1", 1050))
    ...
    it("should validate proxy maxBid", async () => {
      ...
      await expect(handleNewBid(mockCtx, "a1" as Id<"auctions">, "u1", 1100, 1050))
        .rejects.toThrow("Proxy maximum bid must be at least the current bid amount.");
    });

    it("should resolve Case A: someone else has a higher proxy", async () => {
      const auction = { _id: "a1", currentPrice: 1000, minIncrement: 100, status: "active", reservePrice: 500 };
      mockCtx.db.get.mockResolvedValue(auction);
      ...
      mockCtx.db.query = vi.fn().mockImplementation((table) => {
        ...
        return { withIndex: vi.fn().mockReturnThis(), order: vi.fn().mockReturnThis(), filter: vi.fn().mockReturnThis(), first: vi.fn().mockResolvedValue(null) };
      });

      const result = await handleNewBid(mockCtx, "a1" as Id<"auctions">, "u1", 1100);
      
      expect(result.bidAmount).toBe(1200); // u2 outbids u1 by 1 increment
      expect(mockCtx.db.insert).toHaveBeenCalledWith("bids", expect.objectContaining({
        bidderId: "u2",
        amount: 1200,
      }));
    });
    ...
    it("should resolve Case B: current bidder is highest proxy and outbids second highest", async () => {
      ...
      const result = await handleNewBid(mockCtx, "a1" as Id<"auctions">, "u1", 1100, 3000);
    ...
    it("should extend auction if within threshold", async () => {
      const now = Date.now();
      const auction = { _id: "a1", currentPrice: 1000, minIncrement: 100, status: "active", endTime: now + 60000 };
      mockCtx.db.get.mockResolvedValue(auction);
    ...
    it("should NOT extend auction if endTime is missing", async () => {
      const auction = { _id: "a1", currentPrice: 1000, minIncrement: 100, status: "active", endTime: null };
      mockCtx.db.get.mockResolvedValue(auction);
      
      await handleNewBid(mockCtx, "a1" as Id<"auctions">, "u1", 1100);
      
      expect(mockCtx.db.patch).not.toHaveBeenCalledWith("a1", expect.objectContaining({
        isExtended: true,
      }));
    });
  });
