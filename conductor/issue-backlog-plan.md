...
`feat/issue-233-seller-listings-page` (PR #258) → `feat/issue-221-seller-reviews`
(PR #260) → `feat/issue-232-report-profile` (PR #261) →
`feat/issue-231-seller-messaging` (PR #262) → `feat/issue-220-activity-feed`
(PR #263) → `feat/issue-217-og-image` (PR #264) → next issue's branch → ... Each
PR's `--base` is the previous branch in the chain, so its diff on GitHub shows only
that issue's own changes, not the whole stack. **Current tip of the chain:
`feat/issue-217-og-image`** — the next issue branches from there.
...
| 2   | [#221](https://github.com/marcojsmith/AgriBid/issues/221) — seller review/rating system                      | **Done — [PR #260](https://github.com/marcojsmith/AgriBid/pull/260)** (open, awaiting review; base = `feat/issue-233-seller-listings-page`; CodeRabbit review triggered)                                                | Completes the "Seller Rating" trust item (currently hardcoded "No reviews"); other issues' TODO comments already (mistakenly) reference this number, so clearing it first avoids confusion                                                                                                                               |
| 3   | [#232](https://github.com/marcojsmith/AgriBid/issues/232) — report profile functionality                     | **Done — [PR #261](https://github.com/marcojsmith/AgriBid/pull/261)** (open, awaiting review; base = `feat/issue-221-seller-reviews`; CodeRabbit review triggered)                                                      | Trust & safety; the issue explicitly proposes a `profileFlags` table parallel to the existing `auctionFlags` table — small, self-contained                                                                                                                                                                               |
| 4   | [#231](https://github.com/marcojsmith/AgriBid/issues/231) — seller messaging/contact system                  | **Done — [PR #262](https://github.com/marcojsmith/AgriBid/pull/262)** (open, awaiting review; base = `feat/issue-232-report-profile`; CodeRabbit review triggered)                                                      | Larger (new `conversations`/`messages` tables + real-time UI) — do after the two smaller trust-focused issues                                                                                                                                                                                                            |
| 5   | [#220](https://github.com/marcojsmith/AgriBid/issues/220) — user activity feed system                        | **Done — [PR #263](https://github.com/marcojsmith/AgriBid/pull/263)** (open, awaiting review; base = `feat/issue-231-seller-messaging`; CodeRabbit review triggered)                                                    | Replaces the hardcoded `getActivityItems()`; lowest urgency of the #131 sub-issues since the current hardcoded feed doesn't actively mislead users the way "Not linked"/disabled buttons do                                                                                                                              |
| 6   | [#217](https://github.com/marcojsmith/AgriBid/issues/217) — add `og-image.png`                               | **Done — [PR #264](https://github.com/marcojsmith/AgriBid/pull/264)** (open, awaiting review — flagged for human design review, see PR description; base = `feat/issue-220-activity-feed`; CodeRabbit review triggered) | Quick, but needs a **designed image asset**, not just code — an agent can generate a technically-compliant placeholder (1200×630, on-brand colors) programmatically (e.g. an SVG template rendered to PNG), but flag it for human design review rather than treating it as done                                          |
...
| 9   | [#265](https://github.com/marcojsmith/AgriBid/issues/265) — app logo asset + admin-configurable OG image     | Not started                                                                                                                                                                                                             | Follow-up from #217: no canonical logo image exists (header/footer are text-only branding), `businessInfo.logoUrl`'s default `/logo.png` 404s the same way `/og-image.png` did, and the OG image added in #217/PR #264 has no admin customization. Prioritized alongside #171/#253 per explicit instruction (2026-09-07) |
| 10  | [#251](https://github.com/marcojsmith/AgriBid/issues/251) — push/email notifications service                 | Not started                                                                                                                                                                                                             | Infrastructure-heavy (VAPID keys already exist in env — check `.env.local`/`convex env list` for what's already wired before scoping); large, no written implementation plan in the issue yet — needs a planning pass first, not a direct opencode task                                                                  |
| 11  | [#236](https://github.com/marcojsmith/AgriBid/issues/236) — automated screenshot capture                     | **Skip** (explicit instruction, 2026-09-06)                                                                                                                                                                             | Dev/troubleshooting tooling scaffolding, no user-facing or code-quality value — do not implement                                                                                                                                                                                                                         |
| 12  | [#129](https://github.com/marcojsmith/AgriBid/issues/129) — AI chatbot support                               | Not started                                                                                                                                                                                                             | Large, speculative, no implementation plan — needs product scoping before any code task, not a good opencode candidate as-is                                                                                                                                                                                             |
| 13  | [#269](https://github.com/marcojsmith/AgriBid/issues/269) — brand theming settings (font/colors/logo/radius) | Backlog, deliberately not started (2026-09-09)                                                                                                                                                                          | Single-brand marketplace today, not multi-tenant/white-label — a settings UI is premature and would reintroduce the inconsistency the visual refresh is fixing. See issue for full context/future shape. Only pick up if a white-label need shows up.                                                                    |
...
- **Convex `returns` validators throw on undeclared properties** — if a task adds a
  ...
  mention it.
- **Keep design tokens centralized** — font, colors, radius, spacing, shadows belong in `src/index.css`'s `@theme`/`:root`/`.dark` blocks as CSS custom properties, not hardcoded per-component. This keeps future brand-theming settings (backlog item #12 above) a config change instead of a rewrite.
