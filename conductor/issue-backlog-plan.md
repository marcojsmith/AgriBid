...
(PR #263) → next issue's branch → ... Each PR's `--base` is the previous branch in
the chain, so its diff on GitHub shows only that issue's own changes, not the whole
stack. **Current tip of the chain: `feat/issue-220-activity-feed`** — the next issue
branches from there.
