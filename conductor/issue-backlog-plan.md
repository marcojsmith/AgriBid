...
(PR #260) → next issue's branch → ... Each PR's `--base` is the previous branch in the
chain, so its diff on GitHub shows only that issue's own changes, not the whole stack.
**Current tip of the chain: `feat/issue-221-seller-reviews`** — the next issue
branches from there.
...
| 3   | [#232](https://github.com/marcojsmith/AgriBid/issues/232) — report profile functionality         | In progress — feat/issue-232-report-profile                                                                                                                              | Trust & safety; the issue explicitly proposes a `profileFlags` table parallel to the existing `auctionFlags` table — small, self-contained                                                                                                                                                           |
