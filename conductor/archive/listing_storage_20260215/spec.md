...
- **Year**: Required. Integer between 1900 and (Current Year + 1).
  - _Note_: Validation is authoritatively enforced by the server using its current UTC year. Client-side validation is advisory for UX only.
...
- **Summary**: Required _only if_ Service History is true. Otherwise, optional.
