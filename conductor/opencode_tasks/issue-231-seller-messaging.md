...
- **Header nav entry shipped in a follow-up** — the task listed this as a nice-to-have "only if small, low-risk", and it was initially deferred, but it has since been implemented on this branch: both header navigation surfaces (`UserDropdown.tsx` and `MobileMenu.tsx`) now include Messages links with unread-conversation badges, backed by the `getUnreadConversationCount` query in `convex/messages.ts`. Users can also still reach `/messages` via the "New message" notification links (which deep-link to `/messages/<conversationId>`) and the post-contact navigation in Profile.tsx.
