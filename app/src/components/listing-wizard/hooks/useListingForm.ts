...
export const useListingForm = () => {
  ...
  const getStepError = (step: number): string | null => {
    switch (step) {
      case 0:
        if (formData.year < MIN_YEAR)
          return `Please enter a year since ${MIN_YEAR}.`;
        if (formData.location.length <= 2)
          return "Please enter a valid location.";
        if (formData.title.length < TITLE_MIN_LENGTH)
      ...
      case 3: {
        ...
        return hasImages
          ? null
          : "Please upload at least one photo of the equipment.";
      }
      case 4:
        if (formData.startingPrice <= 0)
          return "Starting price must be greater than R 0.";
        if (
