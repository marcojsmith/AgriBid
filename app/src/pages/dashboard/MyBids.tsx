...
/**
 * Produce the status badge metadata (label, visual variant, icon and colour class) for an auction.
 *
 * @param auction - The auction object whose status and flags (e.g., `isWon`, `isWinning`, `isOutbid`, `isCancelled`, `status`) determine the badge
 * @returns A StatusDisplay object containing `label`, `variant`, `icon` and `colorClass` for rendering the auction's status badge
 */
...
/**
 * Render the My Bids page showing the user's current bids, summary statistics,
 * and a filterable, sortable list of auctions with per-auction status and actions.
 *
 * The component displays a loading state, summary cards (active, winning, outbid, total exposure),
 * client-side filtering (All, Winning, Outbid, Ended), client-side sorting (Ending Soon, Recent Activity, Highest Bid),
 * and paginated results with a "Load More" control. Each auction item shows status badges, an optional countdown
 * for active auctions, bid details and an action button that adapts to the auction state.
 *
 * @returns A React element representing the My Bids page.
 */
