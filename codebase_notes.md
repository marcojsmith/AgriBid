...
- **Test quirk to watch**: `getAuctionBidsHandler` (convex/auctions/queries/bids.ts:60) computes `isSeller = auction?.sellerId === auth?.userId`, which evaluates `true` when both sides are `undefined` (missing auction doc + unauthenticated caller). Tests currently rely on this; worth tightening in a future fix.
