...
## Multi-Lot Auctions Rework (2026-09-16)

- **Schema split**: `auctions` is now a scheduled-sale _container_ (title, `startTime`, `endTime`, status `draft|published|closed`, optional fee defaults); the item data lives in `lots` (status `draft|pending_review|approved|assigned|sold|unsold|rejected`). FK tables use `lotId` (`bids`, `proxy_bids`, `lotFees`, `watchlist`, `reviews`, `conversations`, `supportTickets`, `lotFlags`).
- **Liveness is derived, not stored**: a lot is browsable/biddable when `status === "assigned"` **and** its parent auction is `published` and `auction.startTime <= now < (lot.extendedEndTime ?? auction.endTime)`. `getActiveLots` / `useLotLiveWindow` encapsulate this — never branch on a stored `"active"` (it no longer exists).
- **Fee snapshot at assignment**: `assignLotToAuction` copies the auction's `defaultBuyerPremiumPct` / `defaultSellerCommissionPct` onto the lot (`resolvedBuyerPremiumPct` / `resolvedSellerCommissionPct`) so later auction edits can't retroactively change an assigned lot's fees. These resolved defaults are derived at settlement but not persisted to `lotFees` (whose `feeId` is a required `platformFees` FK).
- **Settlement**: `settleLot(ctx, lot, now)` in `convex/auctions/internal.ts` is the single settlement path, shared by the expiry cron (`settleExpiredLots`) and manual container closure (`closeAuctionContainer`). Closing a published container settles its assigned lots first (no stranded `assigned` lots); the cron also auto-closes containers whose window elapsed with no remaining assigned lots.
- **Convex codegen `never`-args fix**: an admin mutation whose handler args were a named interface (`CreateAuctionArgs`) made the generated `FunctionReference` infer `never` args. Fix: declare the validator once as a `v.object({ ... })` const and type the handler via `Infer<typeof validator>`; pass the same const to `mutation({ args: validator })`. Applied to `adminCrud.createAuction`; the old frontend `as unknown as` workaround was removed.
- **Admin UI split**: `/admin/lots` (`AdminLots`, lot moderation table) vs `/admin/auctions` (`AdminAuctions`, container list) + `/admin/auctions/:id` (`AdminAuctionDetail`, lot assignment). Public gallery at `/auctions` links to `/auctions/:id` (`getPublishedAuction`, published/closed only).
- **Rollout complete (2026-09-17)**: staging + production migrated, verified, and reseeded. Post-rollout cleanup done: `convex/migrations/multiLotAuctions.ts` deleted, legacy `lots.startTime`/`endTime` fields dropped from the schema (their indexes `by_end_time`/`by_status_endTime` removed; `admin/statistics.ts`'s recent-sales query now uses a new `by_status_settledAt` index instead). The legacy `auctionFees`/`auctionFlags` tables are no longer in the schema; any leftover rows from before the migration are inert and can be purged opportunistically (no code references them).
