...
- **Schema split**: `auctions` is now a scheduled-sale _container_ (title, `startTime`, `endTime`, status `draft|published|closed`, optional fee defaults); the item data lives in `lots` (status `draft|pending_review|approved|assigned|sold|unsold|rejected`). FK tables use `lotId` (`bids`, `proxy_bids`, `lotFees`, `watchlist`, `reviews`, `conversations`, `supportTickets`, `lotFlags`). The data migration lives in `convex/migrations/multiLotAuctions.ts`.
...
- **Known follow-ups**: once the rework is confirmed in production, delete `convex/migrations/multiLotAuctions.ts`, the empty legacy `auctionFees`/`auctionFlags` tables, and the legacy `lots.startTime`/`endTime` fields.
