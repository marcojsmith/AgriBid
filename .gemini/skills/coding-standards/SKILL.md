...
- **No Suppression:** Never use `eslint-disable` or `@ts-ignore` to bypass type checking. Refactor to fix the underlying type issue instead.
...
- **Cleanliness:** Remove unused `eslint-disable` directives (e.g., those reporting "no problems were reported").
