...
- **Quality Checks:**
    - **Lint:** `cd app && bun run lint`. Fix all errors and warnings.
    - **Test:** `cd app && bun run test`. Ensure all tests pass.
    - **Coverage:** `cd app && bun run coverage`. Ensure thresholds are met.
...
- **Versioning:** Update `app/package.json` according to SemVer (Patch/Minor/Major) BEFORE submitting.
- **Verification:** Run `cd app && bun run build` and `bunx vercel build` locally to ensure production stability.
